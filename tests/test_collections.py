...
class TestBiDirectionalMap:
    ...
    @pytest.mark.parametrize(
        "initial_data, expected_error_msg",
        [
            ([("a", 1), ("b", 1)], r"(?i)value 1 already exists"),
        ],
        ids=["duplicate_value_on_init"],
    )
    def test_initialization_failures(self, initial_data, expected_error_msg):
        """Check for uniqueness validation during initialization."""
        with pytest.raises(ValueError, match=expected_error_msg):
    ...
    def test_initialization_with_duplicate_keys(self):
        """Verify that duplicate keys in initialization data use last value."""
        # This matches standard dict behavior - last value wins
        bimap = BiDirectionalMap([("a", 1), ("a", 2)])
        assert bimap["a"] == 2
        assert bimap.get_key(2) == "a"
        assert len(bimap) == 1

    @pytest.mark.parametrize(
        "key, value, expected_error_msg",
        [
            ("a", 3, r"(?i)key 'a' already exists"),
            ("c", 1, r"(?i)value 1 already exists"),
        ],
        ids=["duplicate_key", "duplicate_value"],
    )
    def test_add_uniqueness_violations(self, key, value, expected_error_msg):
    ...
    def test_set_operations(self):
        ...
        assert bimap.get_key(3) == "a"
        assert not bimap.has_value(1)
        with pytest.raises(ValueError, match=r"(?i)value 3 already exists"):
            bimap.set("c", 3)
    ...
    def test_update_method(self):
        ...
        assert bimap.get_key(3) == "b"
        assert not bimap.has_value(2)
        with pytest.raises(ValueError, match=r"(?i)value 1 already exists"):
            bimap.update({"d": 1})
